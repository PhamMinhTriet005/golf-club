...
import { Tray, CardUser, Loading, Pagination } from '@/components';
...
const DirectoryMembers = () => {
  ...
  const { searchQuery, sortBy, sortDirection } = useOutletContext(); // Get state from Layout
    ...
    const fetchMembers = async () => {
      try {
        ...
        const formatted = result.map(item => ({
            name: `${item.first_name} ${item.last_name}`, // Used for sorting/searching
            raw: {
                userId: item.user_id,
      ...
      <Tray 
        ...
        title={
          <div className="flex items-center gap-2 w-full border-b border-gray-100 pb-4 mb-2 animate-fadeIn">
            ...
            <h2 className="text-2xl font-bold font-outfit text-primary-accent">Club Members</h2>
          </div>
        }
      >
